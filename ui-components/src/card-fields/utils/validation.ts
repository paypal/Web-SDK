...
import luhn10 from "card-validator/dist/luhn-10";
...
export function getActiveElement(): unknown {
...
// Format expiry date
export function formatDate(date: string, prevFormat = ""): string {
  assertString(date);
...
export function filterStyles(rawStyles: any = {}): FieldStyle {
  const camelKey = Object.keys(FIELD_STYLE);
  const dashKey = Object.values(FIELD_STYLE);

  return Object.keys(rawStyles).reduce((acc: any, key: string) => {
...
export function styleToString(style: any = {}): string {
  ...
  return Object.keys(filteredStyles).reduce(
    (acc: string, key: string) =>
      `${acc}  ${camelToDasherize(key)} ${
        typeof style[key] === "object"
          ? `{ ${styleToString(style[key])} }`
          : `: ${style[key]} ;`
      }`,
    ""
  );
...
export function getStyles(style: any): [any, any] {
  return Object.keys(style).reduce(
    (acc: [any, any], key: string) => {
...
export function checkCardNumber(
  value: string,
  cardType: CardType
): { isValid: boolean; isPotentiallyValid: boolean } {
  const trimmedValue = removeSpaces(value);
...
export function checkExpiry(value: string): {
  isValid: boolean;
  isPotentiallyValid: boolean;
} {
...
// export function parseGQLErrors(errorsObject: any): {
//   parsedErrors: [];
//   errors: [];
//   errorsMap: any;
// } {
